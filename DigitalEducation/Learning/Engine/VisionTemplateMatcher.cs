...
{
    public class VisionTemplateMatcher : IDisposable
    {
        private readonly Mat _screenMat;
        private readonly Dictionary<string, Mat> _templateCache = new Dictionary<string, Mat>();

        public VisionTemplateMatcher(Bitmap screenBitmap)
        {
            _screenMat = BitmapToMat(screenBitmap);
        }

        public VisionRecognitionResult FindTemplate(string templatePath, double confidenceThreshold = 0.8)
        {
            ...
            using (var result = new Mat())
            {
                ...
                if (maxVal >= confidenceThreshold)
                {
                    ...
                    return VisionRecognitionResult.Found(location, size, maxVal);
                }
            }
        ...
        public VisionRecognitionResult FindTemplateInRegion(string templatePath, System.Drawing.Rectangle searchRegion, double confidenceThreshold = 0.8)
        {
            ...
            using (var graphics = Graphics.FromImage(regionBitmap))
            {
                ...
                var regionMatcher = new VisionTemplateMatcher(regionBitmap);
        ...
        public List<VisionRecognitionResult> FindAllMatches(string templatePath, double confidenceThreshold = 0.8)
        {
            var results = new List<VisionRecognitionResult>();
        ...
        private VisionRecognitionResult FindTemplateInMat(Mat sourceMat, string templatePath, double confidenceThreshold)
        {
            if (!_templateCache.ContainsKey(templatePath))
            {
                using (var templateImage = new Mat(templatePath, ImreadModes.Color))
