...
{
    public class CustomLessonActionsHandler : ILessonActionHandler
    {
        private readonly Window _ownerWindow;
        private readonly ILessonStorage _storage;
        private readonly string _customLessonsPath;

        public CustomLessonActionsHandler(Window ownerWindow) : this(ownerWindow, new CustomLessonFileStorage())
        {
        }

        public CustomLessonActionsHandler(Window ownerWindow, ILessonStorage storage)
        {
        ...
        public void EditLesson(LessonDataModel lesson)
        {
            try
            {
            ...
            {
                AppDialogService.ShowErrorDialog($"Ошибка при открытии редактора: {ex.Message}", _ownerWindow);
            }
        }

        public void DeleteLesson(LessonDataModel lesson)
        {
            try
            {
                ...
                if (result == true)
                {
                    ...
                    AppDialogService.ShowSuccessDialog("Урок успешно удален!", _ownerWindow);
            ...
            {
                AppDialogService.ShowErrorDialog($"Ошибка при удалении урока: {ex.Message}", _ownerWindow);
            }
        }

        public void StartLesson(LessonDataModel lesson)
        {
            try
            {
            ...
            {
                AppDialogService.ShowErrorDialog($"Не удалось запустить урок: {ex.Message}", _ownerWindow);
            }
        }
